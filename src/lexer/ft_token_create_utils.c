...
/*   Updated: 2024/07/16 16:15:13 by ahors            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    ft_add_token(shell, ft_create_token(TOK_TYPE_CMD, value, quote_status));
}
...
{
    ft_add_token(shell, ft_create_token(TOK_TYPE_ARG, value, quote_status));
}
...
{
    ft_add_token(shell, ft_create_token(TOK_TYPE_REDIR, value, quote_status));
}

// Fonction pour creer et ajouter un token de type pipe
void ft_create_add_pipe_token(t_shell *shell) 
{
    ft_add_token(shell, ft_create_token(TOK_TYPE_PIPE, "|", 0));
}