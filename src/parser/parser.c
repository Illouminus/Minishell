...
char *ft_handle_token_expansion(t_token *current_token, t_shell *shell, int *inside_single_quote) 
{
    *inside_single_quote = 0;
    return ft_expander(ft_clean_token_value(current_token->tok_value, inside_single_quote), shell, *inside_single_quote);
}


void ft_parser_process_token(t_token **current_token, t_shell *shell, t_command **last_command, int *i) 
{
    int inside_single_quote;
    char *cmd_value_clean; 

	cmd_value_clean = ft_handle_token_expansion(*current_token, shell, &inside_single_quote);
