...
/*   Updated: 2024/09/30 11:08:40 by ebaillot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int						ft_parser_handle_redirection(t_token **current_token,
							t_parser_data *data, t_command **last_command);
...
int						handle_pipe_token(t_token **current_token,
							t_parser_data *data);
int						handle_expected_cmd(t_token **current_token,
							t_parser_data *data, char *cmd_value_clean);
int						ft_handle_redirection_token(t_token **current_token,
							t_parser_data *data);
...
void					add_redirection(t_command **cmd, int type,
							char *filename);
int						handle_error_parsing(char *unexpected_token,
							t_parser_data *data, int exit_status,
							int return_status);
...
void					process_segment(const char *segment, char **stack,
							int *top);
char					*combine_paths(const char *current_path,
							const char *path);
...
void					handle_builtin_with_redirections(t_command *cmd,
							t_shell *shell);
int						validate_and_open_redirections(t_command *current,
							t_shell *shell);
